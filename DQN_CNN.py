...
class CarlaEnv:
    ...
    def process(self, image):
        ...
        if self.show_cam:
            cv2.imshow("", i3)
            cv2.waitKey(1)
        i3 = i3.transpose((2, 0, 1)).tolist() # PyTorch expects CHW instead of HWC
...
class DQNAgent:
    ...
    def create_model(self):
        model = R25CamNet(input_shape=(3, IM_HEIGHT, IM_WIDTH))
        return model
...
if __name__ == '__main__':
    ...
    agent.get_qs(np.random.uniform(size=(3, IM_HEIGHT, IM_WIDTH)).astype(np.float32))
